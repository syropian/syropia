...
If we go ahead and run these tests they should be passing. Nice! However, we should also test that after 1 hour, another mail notification will indeed be sent. Fortunately Laravel comes with a `travel` that allows us to travel to a specific time. Let's see what that looks like.
